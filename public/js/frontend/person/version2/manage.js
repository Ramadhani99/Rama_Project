...
function populateCustomer(){
		$('#alert-info').hide();
		$.get('person/customer', function() {
					$('#tbody_person_customer').empty();
				}).done(function(data) {
					data = data.persons;
					$.each(data,function(key,value){
						// console.log(value.name);
						if(value.visibility == 0){
							var tr = '<tr><td id = "name'+value.id+'">'+value.firstname+' '+value.lastname+'</td><td id = "dob'+value.id+'">'+value.dob+'</td><td id = "gender'+value.id+'">'+value.gender+'</td><td id = "type'+value.id+'">'+value.type+'</td>'+
								  '<td>'+
						...
						$('#tbody_person_customer').append(tr);
					}); 
			    }).fail(function() {
				    // alert( "error" );
			    });		
...
function populateStaff(){
		$('#alert-info').hide();
		$.get('person/staff', function() {
					$('#tbody_personstaff').empty();
				}).done(function(data) {
					data = data.persons;
					$.each(data,function(key,value){
						// console.log(value.name);
